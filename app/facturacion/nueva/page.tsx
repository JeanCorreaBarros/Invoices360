...
export default function NuevaFacturaPage() {
    ...
    const handleEmitInvoice = async () => {
        ...
        // Construir payload a partir del estado del componente
        try {
            const payload = {
                orderId: Date.now(),
                orderPrefix: "BAQ",
                orderResolution: "",
                userId: "cf275447-59e7-4b19-a56e-5c57e6a4f1f4",
            ...
            const res = await fetch("/api/modules/factura/invoices", {
                method: "POST",
                headers: myHeaders,
            ...
            if (!invoiceId) {
                console.error("No se recibió el id de la factura:", data)
                throw new Error("No se recibió el id de la factura desde el endpoint")
            }

            // Crear cada item asociado a la factura usando el id recibido
            for (const it of items) {
                ...
                const detailPayload = {
                    orderPrefix: payload.orderPrefix,
                    orderResolution: payload.orderResolution || "",
                ...
                try {
                    const resDetail = await fetch("/api/modules/factura/invoicesDetails", {
                        method: "POST",
                        headers: myHeaders,
                    ...
                    if (!resDetail.ok) {
                        ...
                        console.error(`Error creando item (referencia=${it.referencia}): HTTP ${resDetail.status}: ${txt}`)
                    } else {
                        const d = await resDetail.json().catch(() => null)
                        console.log("Detalle creado:", d)
                    }
                } catch (err) {
                    console.error("Error consumiendo invoicesDetails:", err)
                }
            }

            alert(`Factura emitida correctamente (id: ${invoiceId})`)
            console.log("Emit invoice response:", data)
        } catch (error) {
            console.error("Error emitiendo factura:", error)
            alert("Ocurrió un error al emitir la factura. Revisa la consola para más detalles.")
        } finally {
            setIsEmitting(false)
        }
    }
       ...
       <div className="min-h-screen bg-background flex flex-col">
                ...
                <main className="flex-1 overflow-y-auto p-9 ">
                    ...
                    <motion.div
                        ...
                        className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-sm text-gray-900"
                    >
                        {/* Header */}
                        <motion.div variants={itemVariants} className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex space-x-2">
                                ...
                                <Button variant="outline" className="flex items-center gap-1 bg-white" onClick={handlePrint} disabled={isPrinting}>
                                ...
                                <Button
                                    className="bg-blue-600 hover:bg-blue-700 hover:text-white flex items-center gap-1"
                        ...
                        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                            ...
                            <Dialog open={showSignaturePad} onOpenChange={(open) => { setShowSignaturePad(open); if (open) setTimeout(() => prepareSignatureCanvas(), 50) }}>
                                <DialogContent className="w-[95vw] sm:max-w-[520px] p-4">
                                    <DialogHeader>
                            ...
                            <Dialog open={isNewClientOpen} onOpenChange={(open) => setIsNewClientOpen(open)}>
                                <DialogContent className="w-[95vw] sm:max-w-[520px] max-h-[90vh] overflow-y-auto rounded-xl p-4 sm:p-6 shadow-lg">
                                    ...
                                    <DialogFooter className="flex flex-col sm:flex-row gap-2 mt-4">
                                        ...
                                        }} className="w-full sm:w-auto bg-purple-500 hover:bg-purple-700 text-white">Guardar</Button>
                                    </DialogFooter>
                                </DialogContent>
                            </Dialog>
                        </motion.div>
